...
public class VariantAllSlice {
	private CallGraph cg = null;
	private PointerAnalysis<InstanceKey> pa = null;
	private SDG<InstanceKey> sdg = null;
	
	public VariantAllSlice(CallGraph cg, PointerAnalysis<InstanceKey> pa, DataDependenceOptions ddo, ControlDependenceOptions cdo) {
		if(cg == null || pa == null) {
	...
	public Collection<Statement> mainOnce() {
		...
		try {
			...
			Collection<Statement> slice2 = Slicer.computeBackwardSlice(sdg, s);
			
			for(Statement stat : slice2) {
				if(!slice.contains(stat)) {
					slice.add(stat);
				}
			}
